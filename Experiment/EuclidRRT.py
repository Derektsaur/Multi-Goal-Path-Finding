...
if __name__ == '__main__':
    ...
    """"""
    with open(F'RESULT/EuclidRRT/RESULT_{g}_{len(targetCoordinate)}.csv', 'w', newline='') as csvfile:
    ...
    for testNum in range(1):
        ...
        black_map_gray = cv2.cvtColor(black_map, cv2.COLOR_BGR2GRAY)
        black_map_gray = cv2.cvtColor(black_map, cv2.COLOR_BGR2GRAY)
        route = f'./RESULT/EuclidRRT/TSP_result/{g}_{len(targetCoordinate)}_{testNum}.jpg'
        IMG2 = cv2.imread(r"C:\Users\huang\Desktop\S&Reg V2\PAPER\DRAW\BUILDING\T3851.png")     #T3851  W1200
        route2 = f'./RESULT/EuclidRRT/TSP_result/{g}_{len(targetCoordinate)}_{testNum}_DRAW.jpg'

        cost_total = 0
        for k in range(len(list_final)):
            p = list_final[k][0]
            q = list_final[k][1]
            time_consuming_p_q, cost_p_q, path_p_q, flag_p_q,  node_list_p_q,num_sample_nodes = bias_rrtstar(target_group[p], target_group[q], bias_rate, map_binary, step_size, samplingMax)
            totalSampleNum += num_sample_nodes
            if len(path_p_q)==0:
                break
            draw_Tsp_result(IMG, path_p_q)
            cv2.imwrite(route, IMG)
            draw_Tsp_result(IMG2, path_p_q)
        ...
        print(cost_total)
        with open(F'RESULT/EuclidRRT/RESULT_{g}_{len(targetCoordinate)}.csv', 'a', newline='') as csvfile:
            f = csv.writer(csvfile)
