...
def predict_promising_region_V2(map_route,i,img_gray, net,device):
    ...
    for j in range(1):
        ...
        save_image_tensor2pillow(unit_image2, F"RESULT_DRAW/RegRRT/TRAIN/PREDICTION RESULT/map_{i}_region.jpg")
    ...
    for j in range(1):
        ...
        save_image_tensor2pillow(unit_image2, F"RESULT_DRAW/RegRRT/TRAIN/PREDICTION RESULT/map_{i}_line.jpg")
    for j in range(1):
        ...
        save_image_tensor2pillow(unit_image4, F"RESULT_DRAW/RegRRT/TRAIN/PREDICTION RESULT/map_{i}_line+region.jpg")
...
if __name__ == '__main__':
    ...
    with open(F'RESULT_DRAW/RegRRT/RESULT_{g}_{len(targetCoordinate)}_v2.csv','w',newline='') as csvfile:
    ...
    for testNum in range(10):
        ...
        route_0 = f'./RESULT_DRAW/RegRRT/TRAIN/SPLIT RESULT'
        index = -1
        totalSampleNum = 0
        # original
        ...
        black_map_gray = cv2.cvtColor(black_map, cv2.COLOR_BGR2GRAY)
        route = f'./RESULT_DRAW/RegRRT/TSP_result/{g}_{len(targetCoordinate)}_{testNum}.jpg'
        cost_total = 0
        '''
        ...
        p = 1
        q = 4
        cost, path, flag, node_list,num_sample_nodes,allnode = Region_bias_rrtstar(target_group[p], target_group[q], bias_rate1,bias_rate2, map_binary, step_size, pro_list_final[p][q],pro_list_line[p][q], samplingMax)
        ...
        route = f'./RESULT_DRAW/RegRRT/GROWTH/{g}_{len(targetCoordinate)}_{testNum}.jpg'
        im = Image.fromarray(img).convert('RGB')
        im.save(route, dpi=(300, 300), quality=95)
        # print(g)
        ...
        print(cost_total)
        if failureNum != 0:
            with open(F'RESULT_DRAW/RegRRT/RESULT_{g}_{len(targetCoordinate)}_v2.csv', 'a', newline='') as csvfile:
                f = csv.writer(csvfile)
