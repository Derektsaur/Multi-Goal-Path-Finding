...
def draw_map_with_goal_start(Img, start_node, goal_node):
    ...
    cv2.rectangle(Img, (start_node.y - 3, start_node.x - 3), (start_node.y + 3, start_node.x + 3), (255, 0, 0),
                  thickness=-1)
    cv2.rectangle(Img, (goal_node.y - 3, goal_node.x - 3), (goal_node.y + 3, goal_node.x + 3), (0, 0, 255),
                  thickness=-1)
...
def predict_promising_region_V2(map_route,i,img_gray, net,device):
    ...
    for j in range(1):
        ...
        save_image_tensor2pillow(unit_image2, F"RESULT/RegRRTv1/TRAIN/PREDICTION RESULT/map_{i}_region2.jpg")
    out1=out1.squeeze()
...
def predict_promising_region_V2_DRAW(map_route,i,img_gray, net,device,IMG_GRAY):
    ...
    img_data=transform(img).to(device)
    img_data=torch.unsqueeze(img_data,dim=0)
    IMG_FUSION = cv2.imread(F'RESULT\RegRRTv1\PROMISING REGION\FUSION.jpg')
    ...
    for t in range(len(promising_region_list1)):
        ...
        if len(promising_region_list1)!=0:
            if IMG_GRAY[promising_region_list1[t].x][promising_region_list1[t].y]<=10:
                continue
            # else:
    ...
    im.save(F'RESULT\RegRRTv1\PROMISING REGION/FUSION.jpg', quality=95, dpi=(300.0, 300.0))
    return promising_region_list1,reg,IMG_FUSION
...
if __name__ == '__main__':
    ...
    """"""
    with open(F'RESULT/RegRRTv1/RESULT_{g}_{len(targetCoordinate)}_v1.csv','w',newline='') as csvfile:
    ...
    for testNum in range(1):
        ...
        step_size = 20 # stepsize 15 or 10 only in "map 15"
        ...
        route_0 = f'./RESULT/RegRRTv1/TRAIN/SPLIT RESULT'
        index = -1
        totalSampleNum = 0
        # original
        ...
        route_1 = r'C:\Users\huang\Desktop\S&Reg V2\CODE\RESULT\RegRRTv1\PROMISING REGION'

        IMG_GRAY = copy.copy(map_gray)  # IMG1是带有起点终点的地图， map是只有障碍物的地图
        cv2.imwrite(F"RESULT\RegRRTv1\PROMISING REGION\GRAY.jpg", IMG_GRAY)
        MAP_DRAW = copy.copy(map)
        cv2.imwrite(F"RESULT\RegRRTv1\PROMISING REGION\FUSION.jpg", MAP_DRAW)
        ...
        for i in range(len(list_final)):
            ...
            cv2.imwrite(route_pair_map, IMG1)  # 画下对点图，储存在路径中
            t11 = time.time()
            pro_list1, reg, IMG_FUSION = predict_promising_region_V2_DRAW(route_1, index, img_gray, net, device,
                                                                          IMG_GRAY)
            t22 = time.time()
            im = PIL.Image.fromarray(cv2.cvtColor(IMG_FUSION, cv2.COLOR_BGR2RGB))
            im.save(F"RESULT\RegRRTv1\PROMISING REGION\FUSION.jpg", quality=95, dpi=(300.0, 300.0))
        ...
        for i in range(len(list_final)):
            ...
            cv2.rectangle(IMG_FUSION, (coord1[1] - 6, coord1[0] - 6), (coord1[1] + 6, coord1[0] + 6), (255, 0, 0),
                          thickness=-1)
            cv2.rectangle(IMG_FUSION, (coord1[1] - 6, coord1[0] - 6), (coord1[1] + 6, coord1[0] + 6), (255, 0, 0),
                          thickness=-1)
        cv2.rectangle(IMG_FUSION, (targetCoordinate[0][1] - 6, targetCoordinate[0][0] - 6),
                      (targetCoordinate[0][1] + 6, targetCoordinate[0][0] + 6), (0, 0, 255),
                      thickness=-1)
        img = PIL.Image.fromarray(cv2.cvtColor(IMG_FUSION, cv2.COLOR_BGR2RGB))
        img.save(F"RESULT\RegRRTv1\PROMISING REGION/{g}_FUSION.jpg", quality=95, dpi=(300.0, 300.0))
        ...
        black_map_gray = cv2.cvtColor(black_map, cv2.COLOR_BGR2GRAY)
        black_map_gray = cv2.cvtColor(black_map, cv2.COLOR_BGR2GRAY)
        route = f'./RESULT/RegRRTv1/TSP_result/{g}_{len(targetCoordinate)}_{testNum}.jpg'
        # route2 = f'./RESULT/RegRRTv1/TSP_result/{g}_{len(targetCoordinate)}_{testNum}_DRAW.jpg'
        ...
        for k in range(len(list_final)):
            p = list_final[k][0]
            q = list_final[k][1]
            cost, path, flag, node_list,num_sample_nodes = Region_bias_rrtstar(target_group[p], target_group[q], bias_rate1, map_binary, step_size, pro_list_final[p][q], samplingMax)
            ...
            if not flag:
                ...
                cv2.rectangle(IMG, (targetCoordinate[0][1] - 6, targetCoordinate[0][0] - 6),
                              (targetCoordinate[0][1] + 6, targetCoordinate[0][0] + 6), (0, 0, 255),
                              thickness=-1)
                cv2.imwrite(route, IMG)
        ...
        print(cost_total)
        if failureNum != 0:
            with open(F'RESULT/RegRRTV1/RESULT_{g}_{len(targetCoordinate)}_v1.csv', 'a', newline='') as csvfile:
                f = csv.writer(csvfile)
