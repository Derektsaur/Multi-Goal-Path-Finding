...
def drawRawResult(map,pathRaw,saveSize):
    ...
    # mapSave.save(F"saveImage/mapTSPRawResult.jpg", dpi=(300, 300), bbox_inches='tight', quality=95)
...
def drawFinalResult(newMapFinal, pathPruned, list_final, targetCoordinate, saveSize,testNum,g):
    ...
    for i in range(len(targetCoordinate)):
        cv2.rectangle(newMapFinal, (targetCoordinate[i][1] - 6, targetCoordinate[i][0] - 6),
                      (targetCoordinate[i][1] + 6, targetCoordinate[i][0] + 6), (255, 0, 0), thickness=-1)
    ...
    mapTempSave = mapTempSave.resize((saveSize,saveSize),Image.ANTIALIAS)
    mapTempSave.save(f'./RESULT/SFF/TSP_RESULT/{g}_{len(targetCoordinate)}_{testNum}.jpg', dpi=(300, 300), bbox_inches='tight', quality=95)
    # mapSave = Image.fromarray(cv2.cvtColor(newMapwithNode, cv2.COLOR_BGR2RGB))
    # mapSave = mapSave.resize((saveSize,saveSize),Image.ANTIALIAS)
    # mapSave.save(F"saveImage/mapTSPPrunedResultwithNode.jpg", dpi=(300, 300), bbox_inches='tight', quality=95)
...
if __name__ == '__main__':
    ...
    # c = ((196,196,196),(128,128,128),(55,56,60),(105,105,105),(169,169,169),(153,136,119),(79,79,47),(128,128,128),(128,128,128),(128,128,128),(128,128,128),(128,128,128),(128,128,128),(128,128,128),(128,128,128),(128,128,128))
    ...
    """"""
    with open(F'RESULT/SFF/RESULT_{g}_{len(targetCoordinate)}.csv', 'w', newline='') as csvfile:
    ...
    for testNum in range(10):
        ...
        # 11_4 6 8 10:8 15 25
        # 11_12: 8 13 25
        maxSelect = 5 #5:快 10：慢，但做图好看
        expansionDis = 8
        tresholdTrees = 15 #必须大于等于expansionDis
        sampleNum = 0
        wPruned = 5
        ...
        # for i in range(len(treeList)):
            ...
            #               (targetCoordinate[i][1] + 4, targetCoordinate[i][0] + 4), (0,0,255), thickness=-1)
            # cv2.putText(map,"{}".format(i),(targetCoordinate[i][1] - 5, targetCoordinate[i][0] - 5), cv2.FONT_HERSHEY_COMPLEX, 1, (100, 200, 200), 2)
        # cv2.imshow('a',map)
        # cv2.waitKey()
        ...
        print("Calculation time: {}".format(endTime-startTime))
        totalCost = 0
        ...
        drawFinalResult(newMapFinal, pathPruned, list_final, targetCoordinate, saveSize,testNum,g)

        with open(F'RESULT/SFF/RESULT_{g}_{len(targetCoordinate)}.csv', 'a', newline='') as csvfile:
            f = csv.writer(csvfile)
            f.writerow([testNum, round(endTime-startTime, 2), round(totalCost, 2),
                        sampleNum, elkai.solve_int_matrix(costMatrix)])
