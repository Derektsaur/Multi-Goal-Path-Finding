...
net = Transformer2(**model_args).to(device)
if epoch_num == 3:
    data_path = 'Data_TSP\_Data_debug'
else:
    data_path = 'Data_TSP'
weights_pre = r'figure_result\1020_BN_Decoder\1st/mpt_67.pth'
...
if __name__ == '__main__':
    ...
    while epoch < epoch_num:
        ...
        for (image, seg1, seg2, cost, coord) in tqdm(data_loader, desc='Training', colour='blue'):
            ...
            train_loss = Focal_loss(out_line, seg2)
            opt.zero_grad()
            train_loss.backward()
            opt.step()
            total_Loss += train_loss.item()
            recall, precision, acc, fnr, F1, IOU, DICE = evaluation(out_line, seg2)
    ...
    logger.info(f'-------------------------------\n'
                ...
                f'-------------------------------')
    epoch = epoch - 1
    draw_comparision(ave_Loss, ave_Loss_validate, epoch, label1='training dataset', label2='validate dataset', index_x='epoch', index_y='loss', title='loss-epoch2')
    draw_comparision(ave_F1, ave_F1_validate, epoch, label1='training dataset', label2='validate dataset', index_x='epoch', index_y='F1', title='F1-epoch2')
    ...
    for (image, seg1, seg2, cost, coord) in tqdm(test_loader, desc='Testing', colour='red'):
        with torch.no_grad():
            ...
            for j in range(batch_num_test):
                ...
                predict_img = torch.squeeze(out_line[j])
                ...
                save_image_tensor2pillow(true_img1, F'TestResult/Result1/2_groundtruth_{i}_{j}.jpg')
                ...
                save_image_tensor2pillow(unit_image2, F'TestResult/Result1/2_prediction_{i}_{j}.jpg')
    ...
    logger.info(f"[test results] focal loss:{loss:.4f}, \n"
                f"[GuideLine] recall:{100. * metrics1[0]:.4f}%, "
