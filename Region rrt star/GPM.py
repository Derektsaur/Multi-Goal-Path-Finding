...
class GPM(nn.Module):
    ...
    def forward(self, x):
        x0 = self.conv1_x(x)
        conv2_o = self.conv2_x(x0)
        x0 = self.conv3_x(conv2_o)
        x0 = self.conv4_x(x0)
        x0 = self.conv5_x(x0)
        x0 = self.invep(x0)
        x0 = f.interpolate(x0, scale_factor=4, mode='nearest')
        x0 = self.dc1(torch.cat([conv2_o, x0], dim=1))
        x0 = self.dc2(x0)
        x0 = self.dc3(x0)
        x0 = self.conv3x3(x0)
        out1 = self.Th(self.out(x0))
...
class GPM_pre(nn.Module):
    def __init__(self):
        super(GPM_pre, self).__init__()
...
class GPM_post(nn.Module):
    def __init__(self):
        super(GPM_post, self).__init__()
