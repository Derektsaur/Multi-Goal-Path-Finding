...
if __name__ == '__main__':
    ...
    while epoch < epoch_num:
        ...
        net.train()
        for i, (image, segment_image, coord, CoordNum) in enumerate(data_loader):
            # i : 0~len(data_loader.dataset)/batch_num=len(data_loader)
            # bs=16, 总16000，每次输入16张，共750次循环，每75次输出batch ave loss
            image, segment_image, coord, CoordNum = image.to(device), segment_image.to(device), coord.to(device), CoordNum.to(device)

            out_image, out_coord, out_coordNum = net(image)
            ...
            # train_loss4 = loss_fun3(torch.squeeze(out_coordNum,dim=-1), CoordNum).to(device)
            ...
            total_evaluation_criteria_temp = evaluation(out_image, segment_image)
            ...
            # total_batch_evaluation_criteria_temp = evaluation(out_image, segment_image)
        ...
        for i, (image, segment_image, coord, CoordNum) in enumerate(validate_loader):
            # i : 0~len(data_loader.dataset)/batch_num=len(data_loader)
            # bs=16, 总16000，每次输入16张，共750次循环，每75次输出batch ave loss
            image, segment_image, coord, CoordNum = image.to(device), segment_image.to(device), coord.to(device), CoordNum.to(device)
            with torch.no_grad():
                ...
                train_loss = train_loss1 + train_loss2 + train_loss3
                total_Loss_reg += train_loss_reg

            # Criteria for evaluation
            total_Loss_validate += train_loss.item()

            total_evaluation_criteria_validate_temp = evaluation(out_image, segment_image)
        ...
        loss_reg = total_Loss_reg / (len(validate_loader))
    ...
    with open(F'CSVFile/cost_result1.csv', 'w') as csvfile:
        ...
        f.writerow(["Index", "Coordinate_PRE", "Coordinate_GT","Numb_PRE","Num_GT"])
    total_Loss_test = 0
    ...
    for l, (image, segment_image, coord, CoordNum) in enumerate(test_loader):
        # i : 0~len(data_loader.dataset)/batch_num=len(data_loader)
        # bs=16, 总16000，每次输入16张，共750次循环，每75次输出batch ave loss
        image, segment_image, coord, CoordNum = image.to(device), segment_image.to(device), coord.to(device), CoordNum.to(device)
        with torch.no_grad():
        ...
        for j in range(batch_num_test):
            ...
            with open(F'CSVFile/cost_result1.csv', 'a', newline='') as csvfile:
                ...
                f.writerow(["{}_{}".format(l, j), np.round(torch.reshape(out_coord[j], (-1,)).detach().cpu().numpy(),0),
                            np.round(torch.reshape(coord[j,:,0:2],(-1,)).detach().cpu().numpy(),4),out_coordNum[j].item(),CoordNum[j].item()])
    ...
    if os.path.exists(weights):
        ...
        print('unsuccessfully load weights')
    for l, (image, segment_image, coord, CoordNum) in enumerate(SEENdataloader):
        # i : 0~len(data_loader.dataset)/batch_num=len(data_loader)
        # bs=16, 总16000，每次输入16张，共750次循环，每75次输出batch ave loss
        image, segment_image, coord, CoordNum = image.to(device), segment_image.to(device), coord.to(device), CoordNum.to(device)
        with torch.no_grad():
    ...
    if os.path.exists(weights):
        ...
        print('unsuccessfully load weights')
    for l, (image, segment_image, coord, CoordNum)  in enumerate(UNSEENdataloader):
        # i : 0~len(data_loader.dataset)/batch_num=len(data_loader)
        # bs=16, 总16000，每次输入16张，共750次循环，每75次输出batch ave loss
        image, segment_image, coord, CoordNum = image.to(device), segment_image.to(device), coord.to(device), CoordNum.to(device)
        with torch.no_grad():
