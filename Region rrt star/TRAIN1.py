...
if __name__ == '__main__':
    ...
    while epoch < epoch_num:
        ...
        net.train()
        for i, (image, segment_image, coord) in enumerate(data_loader):
            ...
            if i == len(data_loader) - 1:
                ...
                #     predict_img = out_image[j]
                #     true_img = segment_image[j]
                #     unit_image = torch.stack([predict_img, true_img], dim=0)
                #     save_image(unit_image, F'{save_path}/img{epoch}_{j}_{len(ave_acc)}.png')
        ...
        for i, (image, segment_image, coord) in enumerate(validate_loader):
            # i : 0~len(data_loader.dataset)/batch_num=len(data_loader)
            # bs=16, 总16000，每次输入16张，共750次循环，每75次输出batch ave loss
            image, segment_image, coord = image.to(device), segment_image.to(device), coord.to(device)
            with torch.no_grad():
