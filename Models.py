...
class PositionalEncoding(nn.Module):
    '''Positional encoding 計算角度向量，生成編碼表
    '''
    ...
    def _get_hash_table(self, n_position):
        ...
        return rearrange(torch.arange(n_position), '(h w) -> h w', h=int(np.sqrt(n_position)),
    ...
    def forward(self, x, conv_shape=None):
        ...
        :param x:
        '''
        if conv_shape is None:  # While training
            startH, startW = torch.randint(0, self.n_pos_sqrt - self.train_shape[0], (2,))
            selectIndex = rearrange(
                self.hashIndex[startH:startH + self.train_shape[0], startW:startW + self.train_shape[1]], 'h w->(h w)')
        ...
        # conv_map_shape = enc_output.shape[-2:] 取最右邊的兩個維度 (after CNN, the shape of feature maps)
...
class Encoder(nn.Module):
    ...
    def __init__(self, n_layers, n_heads, d_k, d_v, d_model, d_inner, pad_idx, dropout, n_position, train_shape):
        ...
        self.to_patch_embedding = nn.Sequential(
            nn.Conv2d(3, 6, kernel_size=5), # !channel 由2轉3
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(),
            nn.Conv2d(6, 16, kernel_size=5),
            nn.MaxPool2d(kernel_size=2),
            nn.ReLU(),
            nn.Conv2d(16, d_model, kernel_size=5, stride=4, padding=3) # !stride 5改4
        )
    ...
    def forward(self, input_map, returns_attns=False):
        ...
        '''
        enc_slf_attn_list = []
        enc_output = self.to_patch_embedding(input_map) # size divided by 20 ; channels up to d_model
        ...
        return enc_output,  # [b, 256, 512]

# class Decoder(nn.Module):
#     ''' The Decoder for the neural network module.
#     '''
#
#     def __init__(self, patch_size, n_layers, n_heads, d_k, d_v, d_model, d_inner, pad_idx, stride, n_position,
#                  dropout=0.1):
#         '''
...
#             nn.MaxPool2d(kernel_size=2),
#             nn.ReLU(),
#             Rearrange('(b pad) k p1 p2 -> b pad (k p1 p2)', pad=1),
#             # this is done to ensure compatibility with the Key/Value pair of the decoder
...
class Transformer(nn.Module):
    ...
    def __init__(self, n_layers, n_heads, d_k, d_v, d_model, d_inner, pad_idx, dropout, n_position, train_shape):
        ...
        self.encoder = Encoder(n_layers=n_layers, n_heads=n_heads, d_k=d_k, d_v=d_v, d_model=d_model, d_inner=d_inner,
                               pad_idx=pad_idx, dropout=dropout, n_position=n_position, train_shape=train_shape)
        ...
        self.decoder = nn.Sequential(
            Rearrange('b (h w) d -> b d h w', h=train_shape[0]),  # [b,512,16,16]
            UpSampling(512, 256),
            UpSampling(256, 128),
            UpSampling(128, 64),
            UpSampling(64, 32),
    ...
    def forward(self, input_map):
        x, *_ = self.encoder(input_map)
        # seq_logit = self.classPred(enc_output)
        # print(f'classPred out : {seq_logit.shape}')
...
if __name__ == '__main__':
    tf = Transformer(6, 3, 512, 256, 512, 1024, None, 0.1, 40*40, [16, 16]).to('cuda')
    x = torch.rand([2, 3, 256, 256]).to('cuda')
    print(f'input : {x.shape}')
    out = tf(x)
    print(f'output : {out.shape}')

    summary(tf.to('cuda'), (3, 256, 256))
